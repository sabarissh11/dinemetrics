...
.section-header h2 { margin: 0; font-size: 1.2rem; font-weight: 600; }
.section-badge {
...
def _generate_sample_data():
    ...
    cat_price_range = {
        'Electronics':(500,80000),'Clothing':(200,5000),'Home & Kitchen':(300,15000),
        'Books':(100,800),'Sports':(400,20000),'Beauty':(150,3000),'Toys':(200,4000),'Automotive':(800,50000),
    }
    ...
    df = pd.DataFrame({
        ...
        'unit_price':prices,'quantity':quantities,'discount_pct':discounts,'revenue':revenue,
...
with st.sidebar:
    ...
    st.markdown("---")
    st.markdown("<small style='color:#64748b'>Built with Streamlit + Gemini</small>", unsafe_allow_html=True)
...
with tab1:
    ...
    yoy_rev = None
    if len(sel_years) >= 2:
        max_yr = max(sel_years); prev_yr = max_yr - 1
    ...
    with col3:
        ...
        fig = px.pie(region_data, names='region', values='revenue', hole=0.55,
    ...
    with col4:
        ...
        fig = px.bar(ch_data, x='channel', y='revenue', color='channel',
    ...
    top_prod = df.groupby(['product_name','category']).agg(
        ...
        avg_rating=('rating','mean'), profit=('profit','sum')
...
with tab2:
    ...
    with c1:
        ...
        st.plotly_chart(fig, use_container_width=True)
    with c2:
        fig = px.pie(cat_counts, names=sel_cat, values='count', hole=0.4, title=f'Share: {sel_cat}')
        fig.update_layout(**PLOTLY_THEME, height=350)
        st.plotly_chart(fig, use_container_width=True)

    st.markdown("### 🔗 Bivariate Analysis")
    c1, c2 = st.columns(2)
    with c1: x_col = st.selectbox("X axis", num_cols, key="biv_x")
    with c2: y_col = st.selectbox("Y axis", num_cols, index=1, key="biv_y")
    ...
    Q1 = df[outlier_col].quantile(0.25); Q3 = df[outlier_col].quantile(0.75); IQR = Q3-Q1
    ...
    fig = go.Figure()
    fig.add_trace(go.Box(y=df[outlier_col], name=outlier_col, marker_color='#6366f1',
    ...
    fig = px.line(ts_weekly, x='order_date', y=ts_metric,
                  title=f'Weekly {ts_metric.title()} Trend', color_discrete_sequence=['#6366f1'])
    fig.update_traces(line_width=2)
    fig.add_traces(go.Scatter(x=ts_weekly['order_date'], y=ts_weekly[ts_metric].rolling(4).mean(),
...
with tab3:
    ...
    pivot_cat_region = df.pivot_table(values='revenue', index='category', columns='region', aggfunc='sum').fillna(0)
    fig = px.imshow(pivot_cat_region.round(0), text_auto='.2s', color_continuous_scale='Blues',
                    aspect='auto', title='Revenue Heatmap: Category × Region')
    fig.update_layout(**PLOTLY_THEME, height=400)
    st.plotly_chart(fig, use_container_width=True)
    ...
    rfm['segment'] = pd.cut(rfm['rfm_score'], bins=[2,5,8,12], labels=['At Risk','Regular','Champions'])
    ...
    with c1:
        ...
        fig = px.pie(seg_counts, names='segment', values='count', hole=0.5,
                     color_discrete_sequence=['#ef4444','#f59e0b','#10b981'], title='Customer Segments')
        fig.update_layout(**PLOTLY_THEME, height=320)
        st.plotly_chart(fig, use_container_width=True)
    with c2:
        fig = px.scatter(rfm.sample(min(500,len(rfm))), x='frequency', y='monetary', size='recency',
                         color='segment', opacity=0.7,
                         color_discrete_sequence=['#ef4444','#f59e0b','#10b981'],
                         title='RFM Scatter: Frequency vs Monetary', size_max=15)
        fig.update_layout(**PLOTLY_THEME, height=320)
        st.plotly_chart(fig, use_container_width=True)
    ...
    fig.add_trace(go.Bar(x=qoq['label'], y=qoq['revenue'], name='Revenue', marker_color='#6366f1'), secondary_y=False)
    fig.add_trace(go.Scatter(x=qoq['label'], y=qoq['growth'], mode='lines+markers', name='QoQ Growth %',
                              line=dict(color='#22d3ee',width=2.5), marker=dict(size=7)), secondary_y=True)
    ...
    disc_impact = df.groupby('discount_pct').agg(
        avg_revenue=('revenue','mean'), return_rate=('return_flag','mean'), order_count=('order_id','count')
    ).reset_index()
    ...
    fig = px.scatter(cat_prof, x='revenue', y='profit', size='orders', color='margin', text='category',
                     color_continuous_scale='RdYlGn', size_max=40,
                     title='Revenue vs Profit (bubble=orders, color=margin%)')
...
with tab4:
    ...
    st.markdown("Ask anything about your e-commerce data. Gemini will analyze it in real-time.")
...
When showing numbers, use ₹ for currency and format large numbers clearly."""
    ...
    with chat_placeholder:
        if st.session_state.chat_history:
            ...
            <div style='text-align:center;padding:40px;color:#64748b;'>
                ...
                <div>Ask Gemini anything about your data above</div>
            </div>""", unsafe_allow_html=True)

    user_input = st.text_area(
        "💬 Your question:", height=80,
    ...
    if export_btn and st.session_state.chat_history:
        chat_text = "\n\n".join([f"{'USER' if m['role']=='user' else 'GEMINI'}: {m['content']}"
        ...
        if "gemini_key" not in st.session_state or not st.session_state.gemini_key:
            ...
            with st.spinner("Gemini is analyzing your data..."):
                try:
                    ...
                    history = []
                    for msg in st.session_state.chat_history[:-1]:  # all but last user msg
                    ...
                    if "API_KEY_INVALID" in err or "400" in err:
                        st.error("❌ Invalid Gemini API key. Check it at https://aistudio.google.com/app/apikey")
                    else:
